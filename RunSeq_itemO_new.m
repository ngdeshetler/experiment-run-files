function RunSeq_itemO_new(varargin)
%% SETUP

commandwindow; % bring focus to command window.

script_path=which('RunSeq_itemO_new.m');
...
while strcmp(Settings.phase,'Demo')
    ...
    switch runn
        case 'Rerun'
            RunSeq_itemO_new('settings',Settings,'phase','Demo');
        case 'Move on'
            RunSeq_itemO_new('settings',Settings,'phase','PreScan');
        case 'Quit'
...
switch(Settings.phase{1})
    ...
    case 'PreScan'
        ...
        for h=1:Settings.num_learn_repeats
            ...
            for n=1:length(counterlist.learn_trials)
                ...
                goTime=draw_sequence(PTB, preexposure.sequences{index(n)}, startTime, goTime, image_time, fixation_time);
        ...
        switch runn
            case 'Move on'
                RunSeq_itemO_new('settings',Settings,'phase','Scan Demo');
            case 'Quit'
                Settings.phase{1}='Quit';
        end
        toc
    ...
    case 'Scan Demo'
        ...
        switch runn
            case 'Move on - w/ Trigger'
                RunSeq_itemO_new('settings',Settings,'phase',next_phase,'scan','Yes');
            case 'Move on - w/o Trigger'
                RunSeq_itemO_new('settings',Settings,'phase',next_phase);
            case 'Quit'
                Settings.phase{1}='Quit';
        end
        %%
    case 'N-Back'
        ...
        switch runn
            case 'Move on'
                RunSeq_itemO_new('settings',Settings,'phase','Scan');
            case 'Quit'
                Settings.phase{1}='Quit';
        end
    ...
    case 'Scan'
        ...
        switch runn
            case 'Move on'
                RunSeq_itemO_new('settings',Settings,'phase','Post-scan','scan','No');
            case 'Quit'
                Settings.phase{1}='Quit';
        end
    ...
    case 'Post-scan'
        ...
        fprintf(fileID,['Item Recall data file for %d at %s\n'...
            'Trial\tSequence Ref\tPosition\tOriginal Position\tImage Ref\tChoice\tCorrect\tRT\tTrial Type\tNovel\n'],Settings.SubID,datestr(now));
        ...
        for n=1:length(test_order)
            ...
            fprintf(fileID,'%d\t%d\t%d\t%d\t%d\t%d\t%d\t%.3f\t%d\t%d\n',...
