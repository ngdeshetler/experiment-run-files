...
switch(Settings.phase{1})
    ...
    case 'Post-scan'
        ...
        message=['In this last phase of the experiment we will be testing your memory for the order of the items you '...
            ...
            'and a selection of four items that may or may not have been in that series.  Your job is to determine which '...
            'items were in the series and place them in their correct order, based on how they were presented IN THE '...
            'SCANNING PHASE. You can rearrange the items as you need, and will need to press confirm once you are satisfied '...
            'with your answer'];
        drawMessage_moveOn(PTB,message,'continue instructions')

        message=sprintf(['After ordering the items you will be asked if the order is the same as the order you saw before the '...
        ...
        message=sprintf(['Do you have any questions about the task now that you have practiced it?\n\n'...
            'This phase is all self paced, move through it as you will']);
        drawMessage_moveOn(PTB,message,'begin')
        ...
        for n=1:length(test_order)
            ...
            message='Build the correct sequence based on what you saw IN THE SCANNER';
            theData=sequence_test(PTB,theData,n,counterlist.trial_info(test_order(n)).test.image_names,'scanner_recall',message);
...
for c=1:length(test_images)
    picname=test_images{index(c)};
    pic=imread(char(picname),'jpg');
    moving_img(c)=Screen(PTB.Window,'MakeTexture',pic);
end

% bg_index=randperm(length(bg_images));
% for c=1:length(bg_images)
%     picname=bg_images{bg_index(c)};
%     pic=imread(char(picname),'jpg');
%     bg_img(c)=Screen(PTB.Window,'MakeTexture',pic);
% end

baseRect = [0 0 150 150];
%baseRect_bg=[0 0 200 200];
...
moving_rect{1}=CenterRectOnPointd(baseRect,xCent(5),yCent(5));
...
moving_rect{4}=CenterRectOnPointd(baseRect,xCent(8),yCent(8));
...
% [xCent(12),yCent(12)]=RectCenter([0 target_rect{1}(4) xCenter .75*yMax]);
% bg_rect{1}=CenterRectOnPointd(baseRect_bg,xCent(12),yCent(12));
% bg_rect_mask{1}=CenterRectOnPointd([0 0 100 100],xCent(12),yCent(12));
% [xCent(13),yCent(13)]=RectCenter([.24*xMax target_rect{1}(4) .75*xMax .75*yMax]);
% bg_rect{2}=CenterRectOnPointd(baseRect_bg,xCent(13),yCent(13));
% bg_rect_mask{2}=CenterRectOnPointd([0 0 100 100],xCent(13),yCent(13));
% [xCent(14),yCent(14)]=RectCenter([xCenter target_rect{1}(4) xMax .75*yMax]);
% bg_rect{3}=CenterRectOnPointd(baseRect_bg,xCent(14),yCent(14));
% bg_rect_mask{3}=CenterRectOnPointd([0 0 100 100],xCent(14),yCent(14));
% [xCent(15),yCent(15)]=RectCenter([.25*xMax bg_rect{1}(4) xCenter yMax]);
% bg_rect{4}=CenterRectOnPointd(baseRect_bg,xCent(15),yCent(15));
% bg_rect_mask{4}=CenterRectOnPointd([0 0 100 100],xCent(15),yCent(15));
% [xCent(16),yCent(16)]=RectCenter([xCenter bg_rect{1}(4) .75*xMax yMax]);
% bg_rect{5}=CenterRectOnPointd(baseRect_bg,xCent(16),yCent(16));
% bg_rect_mask{5}=CenterRectOnPointd([0 0 100 100],xCent(16),yCent(16));
...
while ~done
    ...
    for g=length(moving_img):-1:1
        Screen(PTB.Window,'DrawTexture',moving_img(g),[],moving_rect{g});
...
% bg_start=GetSecs;
% answered=false;
% SetMouse(xCenter, bg_rect{2}(2)-10, PTB.Window);
% while ~answered
%     Screen(PTB.Window,'DrawTexture',PTB.target_img(1),[],target_rect{1});
%     Screen(PTB.Window,'FrameRect',tar_color{1},target_rect{1},5);
%     DrawFormattedText(PTB.Window,'Which background did you see with this series IN THE SCANNER?',...
%         'center',moving_rect{2}(4)+20,PTB.textColor);
% 
%     for g=length(moving_img):-1:1
%         if Image(g).placed
%             Screen(PTB.Window,'DrawTexture',moving_img(g),[],moving_rect{g});
...
%     
%     [mx,my,buttons]=GetMouse(PTB.Window);
%     for m=1:length(bg_img)
%         inside_bg=IsInRect(mx,my,bg_rect{m});
%         bg_color=[0 0 0];
%         if inside_bg
%             bg_color=green;
%         end
%         if inside_bg && sum(buttons)>0
%             answered=true;
%             theData.test(n).(test_name).bg_answer=bg_index(m);
%         end
%         Screen(PTB.Window,'DrawTexture',bg_img(m),[],bg_rect{m});
%         Screen(PTB.Window,'FillRect',220,bg_rect_mask{m});
%         Screen(PTB.Window,'FrameRect',bg_color,bg_rect{m},5);
%     end
%     Screen(PTB.Window,'Flip');
% end
% theData.test(n).(test_name).bg_time=GetSecs-bg_start;

theData.(test_name)(n).total_time=GetSecs-startTime;
theData.(test_name)(n).log=log;
close_textures(moving_img);
%close_textures(bg_img);
end
%%
