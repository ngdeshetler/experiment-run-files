function RunSeq_itemO_blocks(varargin)
%% SETUP

commandwindow; % bring focus to command window.

script_path=which('RunSeq_itemO_blocks.m');
...
if nargin==0
    ...
    Formats(4,1).items = {'Yes' 'No' 'Yes - W/O Trigger'};
    
    Prompt(end+1,:) = {'Run N-Back?','nback'};
    Formats(4,2).type = 'list';
    Formats(4,2).style = 'radiobutton';
    Formats(4,2).format = 'text';
    Formats(4,2).items = {'Yes' 'No'};
    ...
    if ~ismember('nback',p.UsingDefaults)
        Settings.nback=p.Results.nback;
    end
end
...
Settings.num_learn_repeats=4;% Number of pre-exposures
...
probe_message{4}='How many items in the sequence are MORE VALUABLE that the item that came before it?';
probe_message{5}='How many items in the sequence are SMALLER than the item that came before it?';

for g=1:length(probe_message)
...
while strcmp(Settings.phase,'Demo')
    ...
    message=sprintf(['In this experiment you will be making judgments about the relationships between items. '...
        ...
        '"More valuable than the previous item".\n\n'...
    ...
    for n=1:length(practice.learn_trials)
        ...
        goTime=draw_sequence(PTB, practice.sequences{index(n)}, startTime, goTime, image_time, fixation_time);
    ...
    switch runn
        case 'Rerun'
            RunSeq_itemO_block('settings',Settings,'phase','Demo');
        case 'Move on'
            RunSeq_itemO_block('settings',Settings,'phase','PreScan');
        case 'Quit'
...
switch(Settings.phase{1})
    ...
    case 'PreScan'
        ...
        %intructions on screen
        message=['This first phase of the experiment will broken into four blocks, each running for approximately 8'...
        ...
        for h=1:Settings.num_learn_repeats
            ...
            index=randperm(length(counterlist.learn_block{h}));%random order of practice presentaion
            ...
            for n=1:length(counterlist.learn_block{h})
                
                if n==floor(length(counterlist.learn_block{h})/2)
                    delay_start=GetSecs;
                ...
                trial_index=((h-1)*length(counterlist.learn_block{h}))+n;
                
                theData.preexposure(trial_index).onset.sequence=goTime;
                theData.preexposure(trial_index).sequence_ref=counterlist.learn_block{h}(index(n));
                
                fprintf(fileID,'%d\t%d\t%d\t%d\t%.3f\t',h,n,trial_index,theData.preexposure(trial_index).sequence_ref,goTime);
                
                goTime=draw_sequence(PTB, preexposure.sequences{counterlist.learn_block{h}(index(n))}, startTime, goTime, image_time, fixation_time);
        ...
        switch runn
            case 'Move on'
                RunSeq_itemO_block('settings',Settings,'phase','Scan Demo');
            case 'Quit'
                Settings.phase{1}='Quit';
        end
        toc
    ...
    case 'Scan Demo'
        ...
        drawMessage_moveOn(PTB,message,'begin')
            ...
            message=['Now we will move onto the scanner portion of the experiment. This phase will be very similar to the '...
            'first phase you just completed. You will be again making judgments about the relationship between an image and the image that '...
            'preceded it. Only one relationship will be used: "Smaller than the previous item." One addition to this phase '...
            'of the experiment is that between each trial you will see a number of arrows. For each arrow indicate the '...
            'direction the arrow is pointing, ''1'' for left, ''2'' for right.  Any questions?\n\nAgain we will start '...
        ...
        if strcmp(Settings.nback,'Yes')
            ...
            'be broken up into five blocks, each about 6 minutes long, with breaks to rest between.']);
        else
            message=sprintf(['Do you have any questions about the task now that you have practiced it?\n\n The task will '...
            'be broken up into five blocks, each about 6 minutes long, with breaks to rest between.']);
        end
        ...
        switch runn
            case 'Move on - w/ Trigger'
                RunSeq_itemO_block('settings',Settings,'phase',next_phase,'scan','Yes');
            case 'Move on - w/o Trigger'
                RunSeq_itemO_block('settings',Settings,'phase',next_phase);
            case 'Quit'
                Settings.phase{1}='Quit';
        end
        %%
    case 'N-Back'
        ...
        switch runn
            case 'Move on'
                RunSeq_itemO_block('settings',Settings,'phase','Scan');
            case 'Quit'
                Settings.phase{1}='Quit';
        end
    ...
    case 'Scan'
        ...
        for m = Settings.block_start:Settings.blocks
            ...
            for n=1:length(block_ref)
                ...
                goTime=draw_sequence(PTB, scanning.sequences{trial_index}, startTime, goTime, image_time, fixation_time);
        ...
        switch runn
            case 'Move on'
                RunSeq_itemO_block('settings',Settings,'phase','Post-scan','scan','No');
            case 'Quit'
                Settings.phase{1}='Quit';
        end
    ...
    case 'Post-scan'
        ...
        %         end
        %
        message=['In this last phase of the experiment we will be testing your memory for the items you saw during the scanning phase. '...
            'For each trial you will be presented with a series of four pictures of the same item. Your job is to determine which picture'...
            ' of that item was shown in the scanning phase.\n\nFor example, an item could be bells, and the pictures would be four '...
            'different bells.  You are to select the bell that was shown to you during the experiment.\n\n Indicate your choice by '...
            'pressing the corresponding number for that picture, 1-4.\n\nIf you are unsure of any answer just make your best guess. '...
            'We will walk through an example together before starting.'];
        drawMessage_moveOn(PTB,message,'begin')
        
        for n=1:length(practice.test_order_item)
            theData.recall_demo(n).image_ref=practice.test_order_item(n);
            theData.recall_demo(n).seq_image_ref=practice.image_test(practice.test_order_item(n)).seq_image_ref;
            theData.recall_demo(n).seq_ref=practice.image_test(practice.test_order_item(n)).seq_ref;
            theData.recall_demo(n).seq_type=practice.image_test(practice.test_order_item(n)).trial_type.number;
            theData=item_test(PTB,theData,n,practice.image_test(practice.test_order_item(n)).image_names,'recall_demo');
        end
        ...
        fprintf(fileID,['Item Recall data -VARIABLE LEARNING BLOCKS- file for %d at %s\n'...
            'Trial\tSequence Ref\tPosition\tOriginal Position\tImage Ref\tChoice\tCorrect\tRT\tTrial Type\tNovel\tStrong_Weak\tLearn Block\n'],Settings.SubID,datestr(now));
        test_order=counterlist.test_order_item;
        ...
        for n=1:length(test_order)
            ...
            theData.image_test(n).image_ref=test_order(n);
            theData.image_test(n).seq_image_ref=counterlist.image_test(test_order(n)).seq_image_ref;
            theData.image_test(n).seq_ref=counterlist.image_test(test_order(n)).seq_ref;
            theData.image_test(n).seq_type=counterlist.image_test(test_order(n)).trial_type.number;
            theData.image_test(n).novel=counterlist.image_test(test_order(n)).novel;
            theData.image_test(n).seq_image_ref_original=counterlist.image_test(test_order(n)).seq_image_ref_original;
            
            theData.image_test(n).learning_block=counterlist.image_test(test_order(n)).learning_block;
            theData.image_test(n).weak_strong.name=counterlist.image_test(test_order(n)).weak_strong.name;
            theData.image_test(n).weak_strong.number=counterlist.image_test(test_order(n)).weak_strong.number;
            
            theData=item_test(PTB,theData,n,counterlist.image_test(test_order(n)).image_names,'image_test');
            
            fprintf(fileID,'%d\t%d\t%d\t%d\t%d\t%d\t%d\t%.3f\t%d\t%d\t%d\t%d\n',...
                n,theData.image_test(n).seq_ref,theData.image_test(n).seq_image_ref,...
            theData.image_test(n).seq_image_ref_original,theData.image_test(n).image_ref,...
            theData.image_test(n).image_choice,theData.image_test(n).image_correct,...
            theData.image_test(n).image_times,theData.image_test(n).seq_type,theData.image_test(n).novel,...
            theData.image_test(n).weak_strong.number,theData.image_test(n).learning_block);
...
%%
function goTime=draw_sequence(PTB, image_list, onset_time, goTime, image_time, fixation_time)
...
%%
function theData=item_test(PTB,theData,n,test_images,test_name)
index=randperm(length(test_images));
theData.(test_name)(n).present_order=index;
for v=1:length(test_images)
    picname=test_images{index(v)};
    pic=imread(char(picname),'jpg');
    test_img(v)=Screen(PTB.Window,'MakeTexture',pic);
end
...
pop_outRect=[exemp_rect{1}(1)-20 exemp_rect{1}(2)-20 exemp_rect{4}(3)+50 exemp_rect{4}(4)+50];
...
Screen('TextSize',PTB.Window,round(xMax/100));
DrawFormattedText(PTB.Window,'Select the image that you saw before',...
    'center',pop_outRect(2)-25,PTB.textColor);
Screen('TextSize',PTB.Window,round(xMax/75));
DrawFormattedText(PTB.Window,'1',...
    xCent_e(1)-2.5,exemp_rect{1}(4)+5,225);
DrawFormattedText(PTB.Window,'2',...
    xCent_e(2)-2.5,exemp_rect{2}(4)+5,225);
DrawFormattedText(PTB.Window,'3',...
    xCent_e(3)-2.5,exemp_rect{3}(4)+5,225);
DrawFormattedText(PTB.Window,'4',...
    xCent_e(4)-2.5,exemp_rect{4}(4)+5,225);

for m=1:length(test_images) %length(exemps{p})
    Screen(PTB.Window,'DrawTexture',test_img(m),[],exemp_rect{m});
    Screen(PTB.Window,'FrameRect',0,exemp_rect{m},5);
end
Screen(PTB.Window,'Flip');
response=str2double(getKey_list(['1','2','3','4'],PTB.kbNum));
theData.(test_name)(n).image_times=GetSecs-item_starts;
theData.(test_name)(n).image_choice_raw=response;
theData.(test_name)(n).image_choice=index(response);
theData.(test_name)(n).image_correct=index(response)==1;

%Draws the screen again with feedback
Screen(PTB.Window,'FillRect',base_grey,pop_outRect);
Screen('TextSize',PTB.Window,round(xMax/100));
DrawFormattedText(PTB.Window,'Select the image that you saw before',...
    'center',pop_outRect(2)-25,PTB.textColor);
...
Screen(PTB.Window,'FrameRect',blue,exemp_rect{response},5);
Screen(PTB.Window,'Flip');
...
%%
function pressed=getKey_list(keys,k)
% Janice Chen 02/01/06
while 1
    while 1
        [keyIsDown,~,keyCode] = KbCheck(k);
    ...
    theAnswer = KbName(keyCode);
    if sum(ismember(keys,theAnswer))==1  % this takes care of numbers too, where pressing 1 results in 1!
        pressed=keys(ismember(keys,theAnswer));
        break
    end
end
end
